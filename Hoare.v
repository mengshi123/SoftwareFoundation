...
Example hoare_asgn_wrong : forall a,
  ...
  {{ fun st => st X = a }}.
Proof. Abort. 
(* This cannot be proven. We choose [a =  X + 1] so
   {{ True }} X ::= a {{ X = a }} becomes {{ True }} X ::= X + 1 {{ X = a }}
    the post condition is false since X cannot equal X + 1.
*)
(** [] *)
...
(*  The above proposition leads to this state:
   (t_update (t_update st X 3) Y (aeval (t_update st X 3) a)) Y = n
   and: [ aeval (update st X 3) a ] -> [ANum 3].
   then [update (t_update st X 3) Y (ANum 3))] updates Y to 3, not n.
   so if n <> 3, the above propostion is not correct.  *)
(** [] *)
...
Proof.
  ...
  - eapply hoare_consequence_pre. apply hoare_asgn.
    unfold bassn, assn_sub, t_update, assert_implies.
    intros st [_ H2]. simpl. apply le_plus_minus. 
    apply leb_iff in H2. simpl in H2. apply H2.
  - eapply hoare_consequence_pre. apply hoare_asgn.
    unfold assn_sub, t_update, assert_implies.
    simpl. intros st[_ H2]. auto.
Qed.
(** [] *)
...
Proof.
  ...
  - intros st st' H H1.
    ...
    + simpl. destruct H1. assumption.
    + unfold not. intros. inversion H0.
  - intros st st' H H1. inversion H; subst. destruct H1.
    unfold bassn in H1. simpl in H1.
    destruct (beq_nat (st' Y) 0) eqn:eqy.
    + apply beq_nat_true in eqy. omega.
    + unfold not in H1. simpl in H1. apply False_ind. apply H1.
      reflexivity.
Qed.
