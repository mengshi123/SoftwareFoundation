...
Proof.
  intros b c.
  intros H.
  assert (HAND: andb true c = c).
  reflexivity.
  assert (HOR: orb false c = c).
  reflexivity.
  destruct b.
  rewrite <- HAND.
  rewrite -> H.
...
Fixpoint bin_to_nat (b: bin) : nat :=
  ...
  | U b' => S ((bin_to_nat b') + (bin_to_nat b'))
  end.

Example test_bin_incr1: bin_to_nat(incr BN) = 1.
Proof. reflexivity. Qed.
Example test_bin_incr2: bin_to_nat(D (incr BN)) = 2.
Proof. reflexivity. Qed.
Example test_bin_incr3: bin_to_nat(U (incr BN)) = 3.
Proof. reflexivity. Qed.
Example test_bin_incr4: bin_to_nat(U (D (incr BN))) = 5.
Proof. reflexivity. Qed.
Example test_bin_incr5: bin_to_nat(D (U (incr BN))) = 6.
Proof. reflexivity. Qed.
(** [] *)

(** $Date: 2017-08-31 11:39:11 -0400 (Thu, 31 Aug 2017) $ *)
