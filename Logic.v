...
Fact not_implies_our_not : forall (P:Prop),
  ~ P -> (forall (Q:Prop), P -> Q).
Proof.
  intros P h Q p. destruct h. apply p.
Qed.
(** [] *)
...
(*  
Proof:
  By definition of ~, we can tranform the theorem to:
      P -> (P -> False) -> False
  Suppose we have P. But we know P -> False, the goal to be proved switch
  to false. Since False leads to any conclusion, we are done.
Qed. *)
(** [] *)
...
Proof.
  intros P Q H.
  unfold not. intros C. intros P1.
  apply H in P1. apply C in P1. inversion P1.
Qed.
(** [] *)
...
(*  Proof.
    By definition of ~ , the goal switch to:
         forall P. (P /\ (P -> False)) -> False.
    So suppose we have P and: [P and (P -> False)], then we
    have to show [P and (P -> False)] is False.
    But if we have P and P implies False then we are left with False. 
    And we are done. *)
(** [] *)
...
Theorem iff_trans : forall P Q R : Prop,
  (P <-> Q) -> (Q <-> R) -> (P <-> R).
Proof.
  intros P Q R [P1a P1b] [Q1a Q1b]. split.
  - intros H. apply P1a in H. apply Q1a in H. apply H.
  - intros H. apply Q1b in H. apply P1b in H. apply H.
Qed.
...
Proof.
  intros Podd Peven n H1 H2. induction n.
    ...
    - simpl. rewrite <- IHl'. unfold tr_rev. simpl.
      ...
      { intros l1. induction l1 as [|x1 l1'].
         ...
         - intros l2. simpl. destruct l2 as [|y l2'].
            ...
            + replace (rev_append l1' [x1]) with (rev_append l1' [] ++ [x1]).
...
Proof.
  intros X test l.
  assert(andb_split: forall (b1 b2 : bool), b1 && b2 = true <-> b1 = true /\ b2 = true).
          {  intros b1 b2. split.
             - destruct b1.
                + destruct b2.
                    * intros _. split. reflexivity. reflexivity.
                ...
                + destruct b2.
                    * intros H. inversion H.
                    * intros H. inversion H. 
             - intros [H11 H22]. rewrite H11. rewrite H22. reflexivity.
          }
  split.
    + induction l as [|x l'].
        - simpl. intros _. apply I.
        - simpl. intros H1.
          apply andb_split in H1. destruct H1. split.
...
(* No, forallb_true_iff verify its correctness, there's nothing else we need
   to worry about. *)
(** [] *)
