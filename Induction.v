...
Proof.
  intros n.  induction n as [| n' IHn'].
    ...
    - (* n = S n' *)
       ...
       simpl.  reflexivity.
       rewrite -> H. rewrite -> IHn'. rewrite -> negb_involutive. reflexivity. Qed.
...
Answer: Induction introduces an induction hypothesis while destruct not.
*)
(** [] *)
...
(** Translate your solution for [plus_comm] into an informal proof:
    ...
    Proof: (
         We suppose natural numbers [n] and [m] be given.  then show:
       ...
       - First, we suppose:
         ...
         Thus, showing [n + 0 = 0 + n] is equivalent to showing:
       ...
       - Next, we suppose:
         ...
         and for some [m'], the hypothesis:

            [n + m'] = [m'+ n] 

         is true. Then we must show that:

            [n + (S m') = (S m') + n].
         By the definition of [+] and with the help of induction hypothesis:
         ...
         which is true according to the therom we have shown before in 
...
(** Write an informal proof of the following theorem, using the
    ...
    We can prove it by induction on [n].
       ...
       We can easily prove it with the help of the definition of:
       ...
       This also can be easily prove by the  induction hypothesis and the definition of 
...
(*b answer:
    That's because the representation of same binary is not unique.
    e.g. the nat 3 has these representations: 
    incr(incr(incr(BN))),U(incr(BN)),incr(D(incr(BN)))...
 *)
...
Lemma incr_double : forall b, incr (double_bin b) = U b.
Proof.
  destruct b.  
    reflexivity.
    reflexivity.
...
Proof.
  induction n as [|n'].
 reflexivity.
 simpl. rewrite IHn'. rewrite <- double_incr. reflexivity.
Qed.
