...
Theorem even5_nonsense :
  ev 5 -> 2 + 2 = 9.
Proof.
  intros H. inversion H. inversion H1. inversion H2. inversion H3.
Qed.
(** [] *)
...
Theorem ev_ev__ev : forall n m,
  ev (n+m) -> ev n -> ev m.
Proof.
  intros n m Hnm Hn. induction Hn as [|n' Hn' IHHn].
...
Proof.
  intros. induction H.
  - apply le_S. apply le_n.
  - apply le_S. apply n_le_m__Sn_le_Sm. apply le_trans with (n := S n).
    + apply le_S. apply le_n.
    + apply H.
Qed.
...
Proof.
  ...
  pose proof le_trans _ _ _ H H0. apply leb_correct. apply H1.
Qed.
...
(* 
   a) [R 1 1 2]     provable. c1-c3 is enough.
      [R 2 2 6] not provable.

   b) no c1-c3 is enough. 

   c) no c1-c3 is enough.
*)
[]
*)
...
(** For example, suppose that we wanted to prove the following
    ...
    [s], then all elements of [s] must occur somewhere in [re].
...
Proof.
  induction l.
  - simpl. intros H. unfold not. intros C. apply C.
  - simpl. destruct (beq_natP n x).
    + simpl. intros H1. unfold not. intros H2. inversion H1.
    + simpl. intros H3. unfold not. intros H4. inversion H4. 
...
(** $Date: 2017-09-23 17:24:15 -0400 (Sat, 23 Sep 2017) $ *)
