...
Proof.
  intros b e1 e2 st st'. split; intros H; inversion H; subst.
  (* -> *) 
      - apply E_IfFalse. simpl. rewrite H5. reflexivity. apply H6.
      - apply E_IfTrue. simpl. rewrite H5. reflexivity. apply H6.
  (* <- *)
      - apply E_IfFalse. apply negb_true_iff in H5.
        apply H5. apply H6.
      - subst. apply E_IfTrue. apply negb_false_iff in H5.
        apply H5. apply H6.
 Qed.
(** [] *)
...
Proof.
  ...
  - (* WHILE *)
    assert (bequiv b (fold_constants_bexp b)). 
    { apply fold_constants_bexp_sound. }
    destruct (fold_constants_bexp b) eqn:Hfold; 
...
Proof.
  ...
  remember (contra {--> 0} {--> 0}) as H. clear HeqH. inversion H.
  remember (E_Skip {--> 0}) as HS. clear HeqHS.
  apply H1 in HS. apply (WHILE_true_nonterm BTrue SKIP {--> 0} {--> 0}).
  - apply refl_bequiv. 
  - assumption.
Qed.
(** [] *)
...
Inductive ceval : com -> state -> state -> Prop :=
  ...
  | E_WhileTrue : forall (st st' st'' : state) (b1 : bexp) (c1 : com),
      ...
      (WHILE b1 DO c1 END) / st' \\ st'' ->
      (WHILE b1 DO c1 END) / st \\ st''
| E_Havoc     : forall (st st' : state) (X : string),
                  CHavoc X / st \\ st'
...
Example havoc_example2 :
  (SKIP;; HAVOC Z) / { --> 0 } \\ { Z --> 42 }.
Proof.
apply (E_Seq SKIP (HAVOC Z) { --> 0 } { --> 0 }(t_update { --> 0 } Z 42)).
    - apply E_Skip.
    - apply E_Havoc.
Qed.
(** [] *)
...
Proof. 
  left. unfold pXY, pYX, cequiv. split; intros; inversion H; subst.
  (* -> *)
   - apply (E_Seq (HAVOC Y) (HAVOC X) st st'0 st').
     + apply E_Havoc. 
     + apply E_Havoc.
  (* <- *)
   - apply (E_Seq (HAVOC X) (HAVOC Y) st st'0 st').
     + apply E_Havoc.
     + apply E_Havoc.
Qed.
(** [] *)
