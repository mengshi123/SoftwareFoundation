...
Proof.
  exists (tsucc (tfalse)). 
  unfold stuck. unfold normal_form. unfold value. split.
...
Proof.
  unfold normal_form, not. intros. destruct H0.
...
Proof with auto.
  ...
  - (* succ *)inversion IHHT.
      + left. unfold value; right. inversion H.
          * inversion H0; subst. solve_by_invert. solve_by_invert.
...
Proof with eauto.
  ...
  induction HE; intros; inversion HT; subst.
    ...
    - apply T_If.
        ...
        + assumption.
        + assumption.
    - apply T_Succ. apply IHHE.  assumption.
    - assumption.
    - inversion H; subst.
        + apply T_Zero.
    ...
    - apply T_True.
    - apply T_False.
    - apply T_Iszero. apply IHHE in H0. assumption.
Qed.
(** [] *)
...
(** Having seen the subject reduction property, one might
    ...
    false. We consider:
        t = [tif ttrue tzero ttrue] and t can step tp
        t' = [0] which is in Nat and is well typed, but t is not well typed.
*)
(** [] *)
...
(** Suppose, instead, that we add this new rule to the [step] relation:
   ...
   this rule?  For each one that does, give a counter-example.
    ...
    Determinism of [step]  tif ttrue ttrue tfalse ==> ttrue/tfalse
...
(** The reduction rule [ST_PredZero] is a bit counter-intuitive: we
    ...
    [step]?  Would doing so create any problems elsewhere?
     ...
     No. it breaks progress property.
     [pred 0] cannot be stepped , so it is a normal form.
     but it is not a value by defination. Thus [pred 0] 
     is well typed but is also stuck.
*)
(** [] *)
