...
(** Suppose we had incorrectly defined subtyping as covariant on both
    ...
    the application gets stuck.)
      ...
      f : obj = obj.gpa
      g : apply the f with a person does not have field gpa.
...
(** Is the following statement true or false?  Briefly explain your
    ...
    forall T,
         ...
         exists S,
            S <: T  /\  S <> T
False.
T := TBase n * TBase n is not a base type. 
But by definition there is no S -> S <: TBase n.
*)
(** [] *)
...
No. Suppose T1  = Top and for some T2 <> Top,
      (Top, T2) <: (T2, Top)                      (S_Top2)
Cannot be true. Because if it is, then we can show Top <: T2
by applying S_Pord to (S_Top2), and we have T2 <> Top, it is 
contradictary!
*)
(** [] *)
