...
Proof.
  ...
  (induction H); simpl; intros; subst;
      try (rewrite aeval_iff_aevalR in H, H0; rewrite H; rewrite H0); 
      reflexivity.
...
Proof.
  intros. generalize dependent st. induction p.
  - intros. exists st. apply E_Skip.
  - intros. exists (t_update st s (aeval st a)). apply E_Ass. reflexivity.
  - intros. inversion H. subst. assert (exists st', p1 / st \\ st') as terminates_fst.
    apply IHp1. assumption. inversion terminates_fst. 
    assert (exists st'', p2 / x \\ st'') as terminates_snd.
    apply IHp2. assumption. inversion terminates_snd. exists x0.
    apply E_Seq with x; assumption.
 -  intros. inversion H. subst.
    assert (exists st', p1 / st \\ st') as terminates_then.
      apply IHp1. assumption.
    inversion terminates_then.
    assert (exists st', p2 / st \\ st') as terminates_else.
        apply IHp2. assumption.
    inversion terminates_else.
    remember (beval st b) as evalb.
    destruct evalb.
    + exists x. apply E_IfTrue. symmetry. auto. auto.
    + exists x0. apply E_IfFalse. symmetry. auto. auto.
 - inversion H.
Qed.
(** [] *)
