...
Proof.
  ...
  - simpl. intros l2. rewrite app_assoc. rewrite IHl1. reflexivity.
Qed.
...
(* When the arguements of the f are functions, it can be useful if [X] and
...
Proof: 
    First we intors all hypothesis and variables. Then move back from context to the formula
    by generalize dependent n. Then induction on l and we get 2 subgoals.

    The first one"forall n : nat, length [ ] = n -> nth_error [ ] n = None" can be easily
    proved by reflexivity.

    Then we need to destruct n and get another 2 minor subgoals.

      The first one "S (length l) = 0 -> Some x = None". And we intros the hypothesis H:
      "S (length l) = 0". And from the defination of nth_erroe. We can know that this 
      can be proved by inversion H.

      The second one"length (x :: l) = S n -> nth_error (x :: l) (S n) = None". In the 
      same way. Intros hypothesis H"length (x :: l) = S n". And we know the induction 
      hypothesis on l, which is "forall n : nat, length l = n -> nth_error l n = None"
      can be applied to this subgoal. And the subgoal becomes "length l = Nat.pred (S n)".
      just inversion on H it switch to "length l = Nat.pred (S (length l))". which can be
      proved by reflexivity.
Qed.
*)
(** [] *)
