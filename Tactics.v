...
(* apply is more widely used in coq. *)
*)
(** [] *)
...
Proof.
  ...
  - intros m eq. induction m.
    + simpl. inversion eq.
    + simpl. apply f_equal. inversion eq. apply IHn. apply H0.
Qed.
(** [] *)
...
(* _Theorem beq_nat_true : forall n m,
    
    beq_nat n m = true -> n = m.

   Proof._: By induction on [n].

    - First, suppose [n = 0].  We must show

       forall m : nat, beq_nat 0 m = true -> 0 = m

       Then we need to intros m and equation as well as induction on m

       to step further. By doing this, we get 2 subgoal, the first one:

          0 = 0

       can be easily proved by reflexivity. The second one:

          0 = S m

       can be easily proved by inversion on eq(beq_nat 0 (S m) = true).

     - Then we need to show in the [n = S n'] case: 
        ...
        can be easily proved by inversion on eq(beq_nat (S n) 0 = true).

        To prove the second one:

           S n = S m

        we need to apply f_equal and the goal we need to prove switch to:

           n = m

        By inversion on eq : beq_nat (S n) (S m) = true we get a new hypothesis H0:

           H0 : beq_nat n m = true

        We get the same goal by apply IHn: forall m : nat, beq_nat n m = true -> n = m

        In the end we just need to apply HO. _Qed_.*)
(** [] *)
...
Proof.
  ...
  - destruct n.
    ...
    + intros H. apply IHl. inversion H. reflexivity.
Qed.
(** [] *)
...
Proof.
  ...
  - simpl. destruct (split l') as [xs ys].
    + intros. inversion H. simpl. assert (IH: combine xs ys = l'). 
      apply IHl'. reflexivity. rewrite IH. reflexivity.
Qed.
(** [] *)
...
Proof.
  intros. destruct b. 
  - destruct (f true) eqn:ft1.
    + rewrite ft1. apply ft1.
    + destruct (f false) eqn:ff1. apply ft1. apply ff1.
  - destruct (f false) eqn:ff2.
     destruct (f true) eqn:ft2.
     apply ft2. apply ff2. rewrite ff2. apply ff2.
Qed.
(** [] *)
...
Proof.
  ...
  - apply beq_nat_true in bnm. rewrite bnm. apply beq_nat_refl.
  - destruct (beq_nat m n) eqn:bmn.
...
(** Give an informal proof of this lemma that corresponds to your
   ...
   Proof:
   we can solve it by destruct on(beq_nat n m) with a extra equality assumption bnm:
   which is bnm : "beq_nat n m = true". And we got 2 subgoals.

   On the "true = beq_nat m n"case. we can apply beq_nat_true in bnm, and the bnm change
   to :n = m. we rewrite bnm in the subgoal and we get another one: "true = beq_nat m m".
   which can be proved by apply beq_nat_refl.

   On the "false = beq_nat m n" case. In the same way, destruct on(beq_nat n m) with 
   a extra equality assumption bmn: "beq_nat m n = true", and bnm switch to 
   "beq_nat n m = false". And we get another minor 2 subgoals.

       first we need to prove "false = true". By rewrite<- bnm. the subgoal1 switch to
       "beq_nat n m = true". In the same way, apply beq_nat_true in bmn. rewrite bmn. 
       and we need to symmetry before apply beq_nat_refl.

       The second minor subgoal "true = true" can be easily proved by reflexivity.

   And we complete this theorem.
...
Proof.
  intros. destruct(beq_nat n m) eqn: bnm.
  - apply beq_nat_true in bnm. apply beq_nat_true in H0. rewrite <-bnm in H0.
    symmetry. rewrite H0. apply beq_nat_refl.
  - destruct(beq_nat m p) eqn: bmp.
    + apply beq_nat_true in bmp. rewrite <- bnm in H. apply beq_nat_true in H.
      rewrite <- H in bmp. rewrite bmp. symmetry. apply beq_nat_refl.
    + rewrite <- bnm in H. rewrite <- bmp in H0. apply beq_nat_true in H.
      apply beq_nat_true in H0. rewrite <- H in H0. rewrite H0. symmetry. 
      apply beq_nat_refl.
Qed.
(** [] *)
...
unfold split_combine_statement.
  ...
  induction l1 as [|h t].
  - simpl. intros l2. destruct l2 as [|h2 t2]. intros l e1 e2. 
    rewrite <- e2. reflexivity. intros l e1 e2. inversion e1.
  - intros l2 l e1 e2. destruct l2 as [|h2 t2]. inversion e1. destruct l as [|p q].
    inversion e2. simpl. simpl in e1. inversion e1. inversion e2.
  assert (H : (split (combine t t2)) = (t, t2)).
  rewrite combine_split with (l:=q). rewrite IHt with (l:=q) (l2:=t2). reflexivity.
  apply H0. apply H2. rewrite IHt with (l:=q) (l2:=t2). reflexivity.
  apply H0. apply H2. rewrite H. reflexivity.
Qed.
...
Proof.
  ...
  - intros lf e. inversion e.
  - intros lf e. simpl in e. destruct (test h) eqn : T.
    + inversion e. rewrite <- H0. rewrite T. reflexivity.
...
Proof.
  ...
  - simpl. destruct (test h) eqn : th.
    + simpl. reflexivity.
    + simpl. intros. replace (negb (fold (fun (x : X) (ans : bool) => andb (negb (test x)) ans) t true)) with (existsb' test t).
     replace (fold (fun (x : X) (ans : bool) => orb (test x) ans) t false) with (existsb test t).
       rewrite IHt. reflexivity. 
(*Here we can see we got the two remaining subgoal can be proved by unfold. But unfold dose
not work.*)
Abort.
(** [] *)
